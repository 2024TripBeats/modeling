        ...
        for _ in range(2):  # 각 날에 대해 두 개의 후보 경로 생성
            ...
            while len(selected_places) < 5:
                ...
                for _, place in df.iterrows():
                    ...
                    # 개방도 고려: 하루에 한 번만 유사도가 낮은 장소 선택
                    if not used_openness:
                        adjusted_similarity = similarity - (user_openness * 0.5)
                        used_openness = True
                    else:
                        adjusted_similarity = similarity

                    score = distance + (1 - adjusted_similarity)
                    if score < best_score:
                        best_score = score
                        best_place = place
            ...
            # 카테고리 5번이나 6번은 하루에 하나만 포함
            count_category_5_or_6 = sum(1 for place in selected_places if place['VISIT_AREA_TYPE_CD'] in [5, 6])
            if count_category_5_or_6 > 1:
                continue
